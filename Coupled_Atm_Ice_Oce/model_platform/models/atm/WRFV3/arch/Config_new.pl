...
while ( <CONFIGURE_DEFAULTS> )
{
  ...
  if ( $latchon == 1 )
  {
	...
	$_ =~ s:CONFIGURE_NETCDF_FLAG:-DNETCDF: ;
	  ...
	  $_ =~ s:CONFIGURE_NETCDF_LIB_PATH:-L\$\(WRF_SRC_ROOT_DIR\)/external/io_netcdf -lwrfio_nf -L\$\(CODEROOT\)/libs/c_coupler/build -lc_coupler -lstdc++ -L$sw_netcdf_path/lib $sw_usenetcdff $sw_usenetcdf : ;
        }
	 }
    else                   
